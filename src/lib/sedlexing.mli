...
val from_var_enc_string: enc ref -> string -> lexbuf
  (** Same as [Sedlexing.from_var_enc_stream] with a string as input. *)

val from_var_enc_channel: enc ref -> in_channel -> lexbuf
  (** Same as [Sedlexing.from_var_enc_stream] with a channel as input. *)
...
(** [Sedlexing.start lexbuf] informs the lexer buffer that any
...
val next: lexbuf -> int
(** [Sedlexing.next lexbuf next] extracts the next code point from the
  lexer buffer and increments to current position. If the input stream
  is exhausted, the function returns [-1]. *)

val mark: lexbuf -> int -> unit
(** [Sedlexing.mark lexbuf i] stores the integer [i] in the internal
  slot. The backtrack position is set to the current position. *)

val backtrack: lexbuf -> int
(** [Sedlexing.backtrack lexbuf] returns the value stored in the
