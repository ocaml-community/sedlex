...
let gen_state lexbuf auto i (trans, final) =
  ...
  let body () =
    pexp_match ~loc
      (appfun (partition_name partition)
         [[%expr Sedlexing_internal.next_int [%e evar ~loc lexbuf]]])
      ( cases
      @ [
          case
            ~lhs:[%pat? _]
            ~guard:None
            ~rhs:[%expr Sedlexing_internal.backtrack [%e evar ~loc lexbuf]];
        ] )
  in
  let ret body =
    [
  ...
  match best_final final with
    ...
    | Some i ->
        ret
          [%expr
            Sedlexing_internal.mark [%e evar ~loc lexbuf] [%e eint ~loc i];
            [%e body ()]]
...
let regexp_of_pattern env =
  ...
  and aux p =
    ...
    match p.ppat_desc with
      ...
      | Ppat_construct ({ txt = Lident "Star" }, Some p) -> Sedlex.rep (aux p)
      | Ppat_construct ({ txt = Lident "Plus" }, Some p) -> Sedlex.plus (aux p)
      | Ppat_construct
          ( { txt = Lident "Rep" },
            Some
              {
      ...
      | Ppat_construct ({ txt = Lident "Chars" }, arg) -> (
          let const =
            match arg with
              | Some { ppat_desc = Ppat_constant const } -> Some const
              | _ -> None
          in
          match const with
