...
let regexp_of_pattern env =
  ...
  and aux p = (* interpret one pattern node *)
    ...
    | Ppat_interval (Pconst_integer (i1, _), Pconst_integer (i2, _)) ->
        Sedlex.chars (Cset.interval
         (codepoint (int_of_string i1)) (codepoint (int_of_string i2)))

    | Ppat_constant (Pconst_string (s, _)) -> regexp_for_string s
    | Ppat_constant (Pconst_char c) -> regexp_for_char c
    | Ppat_constant (Pconst_integer (c,_)) ->
        Sedlex.chars (Cset.singleton (codepoint (int_of_string c)))
    | Ppat_var {txt=x} ->
