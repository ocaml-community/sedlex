...
(* Optimization 6: Dead tag elimination
   ...
   Current: init_mem 1, t0 delayed (Set_prev on 'a'→'b' exit).
...
(* Optimization 7: Self-loop tag delay (Set_prev) [DONE]
   Tags on a self-loop that also appear on all entering transitions
   are delayed to exit transitions as Set_prev.
   Result: init_mem 1, no set_mem on the self-loop, set_mem_prev on exit (O(1)). *)
...
(* Optimization 9: Set_prev with backtracking [DONE]
   Opt at the end means the DFA can accept at two states (with or without
   the optional 'a'). The delayed tag (Set_prev) must survive mark/backtrack
   correctly.
   Result: init_mem 1, set_mem_prev on exit (x: start=0, end=tag0; y: start=tag0, end=lexeme_length). *)
